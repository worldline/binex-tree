 ...
 */
export const extractNodes = (node, depth = -1) => {
 ...
 * @param {Number} nextId = 1- next id used for non-feature nodes.
 * @return {Object} modified data
 */
export const addIds = (data, nextId = 1) => {
 ...
 * @param {String} kind - emulated event kind
 */
export const triggerEvent = (node, kind) => {
 ...
 * @param {Number} duration = 250 - whole operation duration in milliseconds
 */
export const dragNode = (tree, dragged, target, done, duration = 250) => {
  ...
  let triggerNext = () => {
    if (points.length) {
      ...
      tree.svg.selectAll('.droppable').classed('selected', ({x: dx, y: dy, width, height}) =>
        dx <= point.x && point.x <= dx + height && dy <= point.y && point.y <= dy + width
      );
